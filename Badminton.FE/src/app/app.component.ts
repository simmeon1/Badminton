...
@Component({
  selector: 'app-root',
  imports: [Players, MatTabGroup, MatTab, Matches, GameForm],
...
export class App {
  ...
  public readonly responseResource = rxResource<
    Record<number, MatchupCollection>,
    QueryParams | undefined
  >({
    params: () => this.query(),
    stream: (p) =>
      of(this.matchupBuilder.getMatchups(p.params.names, p.params.minGames, p.params.courtCount)),
  });
  ...
  public playersReordered(orderedNames: string[]) {
    ...
    const reorderedCheckboxes: NameCheckbox[] = [
      ...new Set<string>(
        orderedNames.concat(
          this.form
            .names()
