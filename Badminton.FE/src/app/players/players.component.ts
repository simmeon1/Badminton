...
export class Players {
  public readonly displayedColumns: string[] = ['position', 'courtIndex', 'playerIndex', 'name'];
  ...
  public move(movedName: string, currentIndex: number) {
    ...
    const previousIndex = this.dataSource()
      .map((r) => r.name)
      .findIndex((n) => n === movedName);
  ...
  public sortAtoZ() {
    ...
    this.datasourceChanged.emit(
      JSON.stringify(current) === JSON.stringify(sorted) ? sorted.reverse() : sorted,
    );
  }
