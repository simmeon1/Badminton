...
export class Matches {
  ...
  public readonly selectedIndexes = computed(() => {
    ...
    for (let i = 0; i < this.dataSource().length; i++) {
      ...
      const m = row.matchup;
      if (
        [m.pairing1.player1, m.pairing1.player2, m.pairing2.player1, m.pairing2.player2].includes(
          selectedPlayer,
        )
      ) {
        result.push(i);
    ...
    for (const [courtIndex, matchupCollection] of Object.entries(r)) {
      ...
      for (let i = 0; i < matchupCollection.matchups.length; i++) {
        ...
        rows.push({
          ...
          matchupShort: {
            pairing1: {
              player1: getPlayerIndex(m.pairing1.player1),
              player2: getPlayerIndex(m.pairing1.player2),
            },
            pairing2: {
              player1: getPlayerIndex(m.pairing2.player1),
              player2: getPlayerIndex(m.pairing2.player2),
            },
          },
          matchupText: getString(
            m.pairing1.player1,
            m.pairing1.player2,
            m.pairing2.player1,
            m.pairing2.player2,
  ...
  public getIndex(row: MatchupRow) {
    return (
      this.dataSource().indexOf(row) +
      1 -
      this.dataSource().filter((r) => r.courtIndex === row.courtIndex - 1).length
    );
  }
