      ...
      while (true) {
        ...
        const pick = [...queue].sort((a, b) => {
          ...
          if (currentNames.length % 2 !== 0) {
            ...
            const aPairCount = currentPairs.filter(
              (p) =>
                JSON.stringify(getSortedPairing(p.player1, p.player2)) ===
                JSON.stringify(getSortedPairing(lastPlayer, a)),
            ).length;
            const bPairCount = currentPairs.filter(
              (p) =>
                JSON.stringify(getSortedPairing(p.player1, p.player2)) ===
                JSON.stringify(getSortedPairing(lastPlayer, b)),
            ).length;
        ...
        queue = [...nameChunk.slice(nextIndex), ...nameChunk.slice(0, nextIndex)];
